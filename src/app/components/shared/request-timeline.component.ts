            ...
            <li>
              <div class="relative pb-8">
                ...
                <div class="relative flex space-x-3">
                  <div>
                    <span
                      [class]="'flex h-8 w-8 items-center justify-center rounded-full ' + getColorClass(event.type)"
                    >
                      <lucide-icon [img]="getIcon(event.type)" [size]="16"></lucide-icon>
                    </span>
                  </div>
...
export class RequestTimelineComponent {
  ...
  private readonly iconMap: Record<string, typeof CheckCircle> = {
  ...
  getIcon(type: string): typeof CheckCircle {
    return this.iconMap[type] ?? CheckCircle;
  }

  getColorClass(type: string): string {
    return this.colorMap[type] ?? 'bg-gray-100 text-gray-600';
  }
}
