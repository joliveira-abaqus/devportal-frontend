...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, LucideCalendar, LucideArrowRight, CardComponent, StatusBadgeComponent],
  template: `
          ...
          <div class="flex-1">
            ...
            <div class="mt-3 flex items-center gap-4 text-xs text-gray-400 dark:text-gray-500">
              ...
              <span class="inline-flex items-center gap-1">
                <svg lucideCalendar class="h-3 w-3"></svg>
                {{ formatRelativeDateFn(request.createdAt) }}
              </span>
            </div>
          </div>

          <svg lucideArrowRight class="ml-4 h-5 w-5 flex-shrink-0 text-gray-400 dark:text-gray-500"></svg>
        </div>
      </app-card>
    </a>
  `,
...
export class RequestCardComponent {
  ...
  readonly formatRelativeDateFn = formatRelativeDate;
}
