  ...
  template: `
    @if (!events || events.length === 0) {
      <div class="flex items-center gap-2 py-8 text-sm text-gray-500 dark:text-gray-400">
        <lucide-icon [img]="clockIcon" class="h-4 w-4" />
        <span>Nenhum evento registrado ainda.</span>
      </div>
    } @else {
      <div class="flow-root">
        <ul class="-mb-8">
          @for (event of events; track event.id; let isLast = $last) {
            <li>
              <div class="relative pb-8">
                ...
                <div class="relative flex space-x-3">
                  <div>
                    ...
                    >
                      <lucide-icon [img]="getEventIcon(event.type)" class="h-4 w-4" />
                    </span>
                  </div>
                  <div class="flex min-w-0 flex-1 justify-between space-x-4 pt-1.5">
                    <div>
                      ...
                      @if (event.metadata?.['prUrl']) {
                        ...
                        >
                          <lucide-icon [img]="gitPrIcon" class="h-3 w-3" />
                          Ver Pull Request
                        </a>
                      }
                    </div>
...
export class RequestTimelineComponent {
  ...
  readonly gitPrIcon = GitPullRequest;

  private readonly eventIcons: Record<string, typeof CircleCheck> = {
  ...
  getEventIcon(type: string): typeof CircleCheck {
    return this.eventIcons[type] || CircleCheck;
  }
